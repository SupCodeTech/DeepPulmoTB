...
data_pd_ = pd.read_csv('/content/drive/MyDrive/ImageCLEF2022/CVS/caverns_detection_train_bboxes.csv')
...
for files_num in range(1,120):
  ...
  if files_num < 10:
    ...
    data_dir = "CTR_TST_{}".format(files_num)

  nill_file = data_dir

  print("The name of the file to process: " + str(nill_file))

  imgs = nib.load("/content/drive/MyDrive/ImageCLEF2020/Test_masks1/" + nill_file + '.nii.gz')

  newimg = imgs.get_fdata()

  newimg = newimg.transpose(2,1,0)

  src_mask1_ = nib.load("/content/drive/MyDrive/ImageCLEF2020/Test_masks1/" + nill_file + '.nii.gz')

  src_mask1 = src_mask1_.get_fdata()

  src_mask1 = src_mask1.transpose(2,1,0)

  src_mask2_ = nib.load("/content/drive/MyDrive/ImageCLEF2020/Test_masks2/" + nill_file + '.nii.gz')

  src_mask2 = src_mask2_.get_fdata()

  src_mask2 = src_mask2.transpose(2,1,0)

  image_count = getarrayFromslice("/content/drive/MyDrive/ImageCLEF2020/Test_masks2/" + nill_file + '.nii.gz')
  ...
  if select_flag:
    ...
    img_dirs = '/content/drive/MyDrive/ImageCLEF2022/Detection_train_CT/LungMaskv3_2020/' + nill_file
